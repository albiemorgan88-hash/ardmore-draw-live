...
import type { CricketEuropeArticle } from "@/lib/cricketeurope-news";
...
const localArticles: Article[] = [
  {
    ...
    date: "May 2026",
    category: "Coaching",
    excerpt: "Friday nights at The Bleach Green: U11s from 6:00pm, ages 11+ from 7:00–8:00pm. All young players welcome.",
    body: "Junior coaching is on at The Bleach Green on Friday nights. U11s start at 6:00pm, and ages 11+ train from 7:00–8:00pm. All young players are welcome — just come down and join in.",
    image: "/images/news/ardmore-cricket-coaching-friday-nights.jpg",
    imageAlt: "Ardmore Cricket coaching poster with Friday night session times",
  },
  {
    id: 9,
    ...
    date: "May 2026",
    category: "Photos",
    excerpt: "CricketEurope have posted photos from Bonds Glen v Ardmore on 18 May, with a few Ardmore faces in the gallery.",
    body: "CricketEurope have posted a photo gallery from Bonds Glen v Ardmore on 18 May. There are shots of Conor King, Matt Cuffe, Vivek Sheoran, Phil Patterson and more from the afternoon.",
    source: "CricketEurope",
    sourceUrl: "https://www.cricketeurope.com/PHOTOS/2026/ALBUMS/424316.shtml",
  },
  {
    id: 8,
    ...
    date: "May 2026",
    category: "History",
    excerpt: "Fifteen archive photographs and newspaper clippings from the club archive, including Ardmore 2nd XI, Raman Lamba and trophy presentations.",
    body: "",
    link: "/news/ardmore-archive-clippings",
    source: "Club archive",
  },
  {
    id: 7,
...
const categoryColors: Record<string, string> = {
  ...
  "Match Reports": "bg-sky-100 text-sky-800",
  "Photos": "bg-sky-100 text-sky-800",
  "CricketEurope": "bg-sky-100 text-sky-800",
  "Cup": "bg-emerald-100 text-emerald-800",
};

export default function NewsPageClient({ cricketEuropeArticles = [] }: { cricketEuropeArticles?: CricketEuropeArticle[] }) {
            ...
            {filtered.map(article => (
              <article
                ...
                className={`bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden transition-all ${
                  article.featured || article.id === 10 || article.id === 9 || article.id === 8 || article.id === 7 || article.id === 6 || article.id === 1 || article.id === 2 ? "md:col-span-2" : ""
                }`}
              >
                <div className="p-6">
                  ...
                  {article.image && (
                    <Image
                      ...
                      width={900}
                      height={1200}
                      className="mb-5 w-full max-h-[520px] rounded-lg object-contain bg-navy/5"
                    />
                  )}
                  ...
                  {article.link ? (
                    ...
                    <>
                      ...
                      {article.source && (
                        <p className="text-sm text-navy/40 italic mb-3">
                          Source: {article.sourceUrl ? <a href={article.sourceUrl} className="hover:underline">{article.source}</a> : article.source}
                        </p>
                      )}
                      <button
