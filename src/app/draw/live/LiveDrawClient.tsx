...
function hasCompleteWinners(draw: DrawResult) {
  return Array.isArray(draw.winners) && draw.winners.length >= 3;
}

function isSameLocalDate(isoDate: string, date: Date) {
  return new Date(isoDate).toDateString() === date.toDateString();
}

function createTestDrawData(): DrawResult {
  return {
    draw_number: 999,
...
export default function LiveDrawClient() {
  ...
  // Fetch draw results. During Friday polling, only animate a completed draw
  // from the current local date so last week's result cannot fire early.
  const fetchResults = useCallback(async (options?: { animateNew?: boolean; showLatest?: boolean }) => {
    try {
      const response = await fetch('/api/draw/results');
      if (response.ok) {
        const data = await response.json();
        if (data.results && data.results.length > 0) {
          const latestDraw = data.results[0] as DrawResult;
          const shouldAnimate =
            options?.animateNew &&
            latestDraw.draw_number > lastKnownDraw &&
            hasCompleteWinners(latestDraw) &&
            isSameLocalDate(latestDraw.drawn_at, new Date());
  ...
  useEffect(() => {
    ...
    if (isTestMode) {
      ...
      setTimeout(() => {
        setDrawResult(createTestDrawData());
        setLastKnownDraw(999);
        setAnimationStage('suspense');
      }, 2000);
    } else {
      fetchResults({ showLatest: !isToday });
    }
  }, [fetchPot, fetchResults, isTestMode, isToday]);

  // Polling logic
  useEffect(() => {
    if (!isTestMode && hasBaseline && (isPreDrawPolling || isDrawTime) && !drawResult) {
      setIsPolling(true);
      const timers: {
      ...
      const poll = async () => {
        const found = await fetchResults({ animateNew: true });
        if (found) {
          setIsPolling(false);
          stopPolling();
        }
  ...
  }, [isPreDrawPolling, isDrawTime, drawResult, fetchResults, isTestMode, hasBaseline]);
  ...
  if (animationStage === 'ball1' || animationStage === 'ball2' || animationStage === 'ball3') {
    ...
    if (!winner) {
      return (
        <div className="min-h-screen bg-gradient-to-b from-slate-900 to-slate-800 flex items-center justify-center p-4">
          <div className="text-center">
            <h1 className="text-3xl font-bold text-white mb-4">Results are being finalised</h1>
            <p className="text-xl text-gray-300">Check back shortly.</p>
          </div>
        </div>
      );
    }
      ...
      <div className="min-h-screen bg-gradient-to-b from-slate-900 to-slate-800 flex flex-col items-center justify-center p-4 text-center relative overflow-hidden">
        ...
        <div className="bg-slate-700 rounded-lg p-4 max-w-md w-full mb-8 text-sm">
          <p className="text-gray-300">
            ...
            Club Share: £{((drawResult.pot_amount * 0.4) / 100).toFixed(2)} • 
            Platform: £{((drawResult.pot_amount * 0.075) / 100).toFixed(2)}
          </p>
        </div>

        <p className="text-lg text-gray-300 mb-6">
          Winners have also been emailed
        </p>
